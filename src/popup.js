...
document.addEventListener('DOMContentLoaded', () => {
  ...
  browser.storage.local.get('forceSettings').then((result) => {
    if (result.forceSettings) {
      ...
      repelForceSlider.value = Math.min(100, Math.max(0, (Math.abs(forceSettings.repelForce) / 10)));
  ...
  centerForceSlider.addEventListener('input', () => {
    ...
    if (simulation) {
      // Center force needs to be recreated with a stronger influence
      const container = document.getElementById('graph-container');
      const width = container.clientWidth;
      const height = container.clientHeight;
      
      // Remove existing center force and add a new one
      simulation.force('center', null);
      
      // Create a much stronger center force by using a custom force implementation
      // that applies a stronger pull based on the slider value
      simulation.force('center', function(alpha) {
        // Apply a significantly stronger centering effect
        // Multiply by a large factor (50) to overcome the repel force
        const centerStrength = alpha * value * 50;
        
        return function(d) {
          d.vx += (width/2 - d.x) * centerStrength;
          d.vy += (height/2 - d.y) * centerStrength;
        };
      });
...
function renderGraph(graphData, viewSettings = {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Use a custom center force implementation with much stronger pull
    .force('center', function(alpha) {
      // Apply a significantly stronger centering effect
      // Multiply by a large factor (50) to overcome the repel force
      const centerStrength = alpha * forceSettings.centerForce * 50;
